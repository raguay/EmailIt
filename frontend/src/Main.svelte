<script>
    ...
    let notestruct = {
        ...
        loadNotes: async function () {
            ...
            //
            let notejsonloc = await App.AppendPath(
                $config.configDir,
                "notes.json"
            );
      ...
      rt.EventsOn("scriptRun", async (msg) => {
        ...
        rt.EventsOn("emailSend", async (msg) => {
            ...
            if (msg.account === "") {
                ...
                //
                let acc = $emailaccounts.filter(
                    (item) => item.name === msg.account
                );
                if (acc.length > 0) {
                    acc = acc[0];
    ...
    function runTemplate(template, text) {
        ...
        try {
            ...
            //
            var defaultData = $templates.find(
                (item) => item.name === "Defaults"
            );
            if (template === "given") {
                template = text;
                data["text"] = "";
   ...
   "office": {
    ...
    async function createDefaultConfig(homedir, configdir) {
        ...
        let configloc = await App.AppendPath(configdir, "config.json");
        let emailaccountloc = await App.AppendPath(
            configdir,
            "emailacounts.json"
        );
    ...
    async function getTemplatesList() {
        let templateloc = await App.AppendPath(
            $config.configDir,
            "templates.json"
        );
